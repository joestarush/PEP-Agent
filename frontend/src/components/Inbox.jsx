      ...
      <div className="flex justify-between items-center mb-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {isLoading ? 'Resetting...' : 'Reset Inbox'}
          </button>
          <button
          ...
          >
            {isProcessing ? <RefreshCw size={16} className="animate-spin" /> : <Zap size={16} fill="currentColor" />}
            {isProcessing ? 'AI Thinking...' : 'Analyze Unread'}
          </button>
        </div>
      </div>
                ...
                <div className="flex items-start gap-4">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold text-white shrink-0 ${
                      email.sender.includes('boss') ? 'bg-blue-500' : email.sender.includes('spam') ? 'bg-red-500' : 'bg-emerald-500'
                    }`}
                  >
